					...
					if (Input.GetTouch (0).phase == TouchPhase.Began) {
						...
						if (counter > 2) {
							...
							if (rightleft) {
								...
								var up = Vector3.up;
								this.transform.parent.gameObject.transform.RotateAround (Vector3.zero, up, angle);
							} else if (topdown) {
								var angle = (secondpoint.y - firstpoint.y);
								if (angle < 0) {
									angle = 360 + angle;
								}
								var right = Vector3.Cross (this.transform.parent.gameObject.transform.position, Vector3.up);
								right = this.transform.parent.gameObject.transform.right;
								this.transform.parent.gameObject.transform.RotateAround (Vector3.zero, right, angle);
							}
						} else {
							counter++;
						}
